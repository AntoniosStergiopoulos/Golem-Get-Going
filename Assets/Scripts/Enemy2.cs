...
public class Enemy2 : MonoBehaviour
{
    ...
    {
        Collider2D attackPlayer = Physics2D.OverlapCircle(transform.position, attackRange, whatIsPlayer);
    ...
    private void CheckIfHittingPlayer() 
    {
        ...
        {
            Collider2D hitPlayer = Physics2D.OverlapCircle(attackPoint.transform.position, attackPointRange, whatIsPlayer);
            if (hitPlayer != null)
            {
                float dir = transform.position.x - hitPlayer.transform.position.x;
                hitPlayer.gameObject.SendMessage("ReceiveDamageFromEnemyScript", dir);
                canHitPlayer = false;
                break;
            }
        }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (attackPoints != null)
        {
            foreach (var attackPoint in attackPoints)
            {
                Gizmos.DrawWireSphere(attackPoint.transform.position, attackPointRange);
            }
        }
